...
(function () {
  ...
  function Frame(b, o) {
    ...
    const F = {
      ...
      /** Legacy `blockLamp`, now authored as one falling destructible. */
      lamp(px, pz, h) {
        const A=window.DestructibleAuthoring;
        if(A)A.add('neon',{kind:'serviceStreetLamp',x:wx(px,pz),y:y0,z:wz(px,pz),ry:rot,s:h/17});
        else{F.cyl(KEY.post,px,h*.5,pz,h);F.box(px,h+.25,pz,2.8,.55,1.2,C.neonGold);F.col(px,pz,1.6,1.6,h,0);}
        return F;
      }
    };
    return F;
