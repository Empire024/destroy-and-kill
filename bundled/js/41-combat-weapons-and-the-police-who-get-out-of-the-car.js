...
(function () {
  ...
  function playGunshot(ctx,id,scale){
    ...
    const body=ac.createOscillator(),bg=ac.createGain();body.type='sine';body.frequency.setValueAtTime(id==='shotgun'?112:id==='rifle'?142:id==='smg'?188:164,t);body.frequency.exponentialRampToValueAtTime(id==='shotgun'?34:id==='rifle'?48:id==='smg'?68:56,t+(id==='shotgun'?.09:.055));bg.gain.setValueAtTime((id==='shotgun'?.36:id==='rifle'?.27:id==='smg'?.17:.24)*s,t);bg.gain.exponentialRampToValueAtTime(.0001,t+(id==='shotgun'?.18:id==='rifle'?.12:.10));body.connect(bg);bg.connect(master);body.start(t);body.stop(t+.20);
    const crk=ac.createBufferSource(),chp=ac.createBiquadFilter(),crg=ac.createGain();crk.buffer=ensureGunNoise(ac);chp.type='highpass';chp.frequency.value=id==='shotgun'?2100:id==='rifle'?2600:id==='smg'?3200:2900;crg.gain.setValueAtTime((id==='shotgun'?.15:id==='rifle'?.19:id==='smg'?.12:.16)*s,t);crg.gain.exponentialRampToValueAtTime(.0001,t+.028);crk.connect(chp);chp.connect(crg);crg.connect(master);crk.start(t,Math.random()*.04);crk.stop(t+.05);
  ...
  window.GAME_DEBUG_GUNSHOT_AUDIO=function(id,s){try{if(!ctxRef)return 'no-ctx';playGunshot(ctxRef,id,s==null?1:s);return 'ok';}catch(e){return 'ERR:'+e;}};
  ...
  function officerShoot(ctx,of){
    ...
    const moving=onFoot?playerSpeed>3.5:ctx.player.mph>7,skill=of.profile?of.profile.accuracy:.75,level=tune.level||ctx.stats.wanted||1,suppressed=clamp(c.suppression||0,0,1),chance=clamp((.055+level*.032+(moving?-.085:.03)-clamp(d/120,0,1)*.17+(skill-.75)*.15-suppressed*.26)*(playerCrouched?.6:1)*(performance.now()<(window._nsCopGraceUntil||0)?.35:1),.02,.34);
    ...
    impact(ctx,px-ux*1.5,py,pz-uz*1.5,0xff8a4b);window._nsCopGraceUntil=Math.max(window._nsCopGraceUntil||0,performance.now()+(onFoot?1050:620));const damage=(3.1+level*.72+(tune.aggression||1)*.45)*.68,vd=GameSystems.api('vdamage');if(!onFoot&&level>=3&&Math.random()<.13+level*.022&&ctx.engine.vehicleTireCorners&&ctx.engine.burstTire){const dy=py-oy,len3=Math.hypot(dx,dy,dz)||1,vx=dx/len3,vy=dy/len3,vz=dz/len3;let tireHit=null,tireT=len3;for(const p of ctx.engine.vehicleTireCorners('player')){if(p.burst)continue;const t=raySphereT(of.x,oy,of.z,vx,vy,vz,p.x,p.y,p.z,.9,tireT);if(t>=0&&t<tireT){tireT=t;tireHit=p.key;}}if(tireHit)ctx.engine.burstTire(tireHit,'POLICE GUNFIRE');}
