...
`;
  ...
  function buildPanel() {
    ...
    const tabBtns = {};
    for (const [key, label] of [['tune', 'TUNE & ARMOUR'], ['buy', 'BUY A CAR']]) {
    ...
    ui = { h2, wallet, tabs: tabBtns, cars, pane, tab: 'tune', car: null };
  }
  ...
  function renderTune(){
    const id=ui.car,e=prog.entry(id),profile=prog.upgradeProfile(id);const h3=document.createElement('h3');h3.textContent='TUNE — '+(e?e.displayName:'');ui.pane.appendChild(h3);
    const engine=document.createElement('p');engine.className='hint';engine.textContent=(profile.engineName||'ENGINE').toUpperCase()+' · '+(profile.forcedInduction==='na'?'NATURALLY ASPIRATED':String(profile.forcedInduction).toUpperCase())+' · STAGE '+profile.maxStage+' MAX · SAFE '+Math.round(profile.safeRpm)+' RPM';ui.pane.appendChild(engine);
    const wrap=document.createElement('div');wrap.className='presets';const allowedRaw=e&&Array.isArray(e.tunePresets)&&e.tunePresets.length?e.tunePresets:['stock'],allowed=allowedRaw.filter(pid=>!prog.presetAllowed||prog.presetAllowed(id,pid)),cur=pendingPreset[id]!=null?pendingPreset[id]:prog.presetOf(id);
    for(const pid of allowed){const def=prog.presets().find(p=>p.id===pid);if(!def)continue;const b=document.createElement('button');b.type='button';if(pid===cur)b.classList.add('on');const installed=prog.presetInstalled?prog.presetInstalled(id,pid):true,cost=prog.presetCost?prog.presetCost(id,pid):0,parts=prog.presetHardware?prog.presetHardware(id,pid):(def.hardware||[]);const nm=document.createElement('b');nm.textContent=def.name+(installed?' · INSTALLED':cost?' · '+nf(cost):'');const ds=document.createElement('span');ds.textContent=(def.desc||'')+(parts.length?' Hardware: '+parts.join(', ')+'.':'');const fx=document.createElement('i'),eff=prog.presetEffect(id,pid)||{};fx.textContent=Object.keys(eff).filter(f=>eff[f].pct!==0).map(f=>f+' '+(eff[f].pct>0?'+':'')+eff[f].pct+'%').join('   ')||'factory settings';b.append(nm,ds,fx);b.addEventListener('click',()=>{pendingPreset[id]=pid;prog.setPreset(id,pid,{preview:true});render();});wrap.appendChild(b);}
    ui.pane.appendChild(wrap);const p=document.createElement('p');p.className='hint';p.textContent='CONFIRM purchases new hardware once. Installed packages can be swapped freely. Extreme ECU remains available on unsuitable cars, but heat and reliability penalties are real.';ui.pane.appendChild(p);const protect=GameSystems.api('vehicleUpgrades');if(protect&&protect.renderBodyShop)protect.renderBodyShop(ui.pane,id);
  }
  ...
  function openPanel(rec) {
    ...
    render();
    keyHandler = ev => { if(window.OV_TEXT_ENTRY&&window.OV_TEXT_ENTRY()&&(ev.key||'').toLowerCase()!=='escape')return;
  ...
  function closePanel(confirm){
    if(!openShop)return;const rec=openShop;
    if(confirm){let total=0;for(const id of Object.keys(pendingPreset||{}))total+=prog.presetCost?prog.presetCost(id,pendingPreset[id]):0;if(total>prog.wallet()){toast('✖ NEED '+nf(total-prog.wallet())+' FOR SELECTED HARDWARE','#ff6b6b');return;}}
    openShop=null;root.classList.remove('open');document.body.classList.remove('shop-open');if(keyHandler){removeEventListener('keydown',keyHandler,true);keyHandler=null;}
    if(confirm){let changed=0;for(const id of Object.keys(pendingPaint)){prog.setPaint(id,pendingPaint[id]);changed++;}for(const id of Object.keys(pendingPreset)){const r=prog.installPreset?prog.installPreset(id,pendingPreset[id]):{ok:prog.setPreset(id,pendingPreset[id])};if(r.ok)changed++;else toast('✖ '+r.reason,'#ff6b6b');}if(changed)toast('✓ '+rec.def.name+' — hardware and paint saved',hexs(rec.def.style.accent));}
    else{for(const id of Object.keys(pendingPreset))prog.setPreset(id,entryPresets[id]||'stock',{preview:true});if(entryVehicle&&prog.currentVehicle()!==entryVehicle)prog.selectVehicle(entryVehicle);const cur=prog.currentVehicle();if(cur)ctx.vehicles.setColor(prog.paintOf(cur));if(Object.keys(pendingPaint).length||Object.keys(pendingPreset).length)toast('Left as it was','#9ab');}
    pendingPaint=pendingPreset=entryPresets=null;GameSystems.events.emit('shop:exit',{id:rec.def.id,confirmed:!!confirm});
  }
  ...
  function runOverMechanic(rec) {
    ...
    try { const crime=GameSystems.api('crime');if(crime)crime.report('assault-mechanic',{perpetrator:'player',actor:ctx.player,x:rec.def.x,z:rec.def.z,severity:2,immediate:true}); } catch (e) { console.error('[bodyshop] crime report failed', e); }
    let cops = 0;
