...
(function () {
  ...
  const RUBBER_BAND = 0.08;        // default; individual races may tune this by ±2%
  const WRONG_WAY_COS = -0.18;     // sustained heading disagreement before warning
  const WRONG_WAY_GRACE = 0.72;
  const SPLIT_SHOW_TIME = 3.4;
  ...
  const races = {
    ...
    ui: null, pool: [], crewPool: [], rings: null, confirmAbandon: 0, retry: null,
    trafficExclusion: null, countdownClearClock: 0
  };
  ...
  function makeCheckpointArrow() {
    ...
    const mesh=new THREE.Mesh(new THREE.PlaneGeometry(.82,.82),mat);
    mesh.position.set(0,.82,.045);mesh.renderOrder=50;mesh.visible=false;
    mesh.userData.canvas=canvas;mesh.userData.tex=tex;mesh.userData.key='';
    return mesh;
  }
  function paintCheckpointArrow(mesh,turn) {
    if(!mesh)return;
    if(!turn||turn.key==='straight'){mesh.visible=false;mesh.userData.key='straight';return;}
    mesh.visible=true;
    if(mesh.userData.key===turn.key)return;
    mesh.userData.key=turn.key;
    const cv=mesh.userData.canvas,g=cv.getContext('2d');g.clearRect(0,0,cv.width,cv.height);
    g.textAlign='center';g.textBaseline='middle';g.font='900 176px system-ui,Segoe UI Symbol,sans-serif';
    g.lineJoin='round';g.lineWidth=24;g.strokeStyle='rgba(0,0,0,.78)';g.shadowColor='rgba(255,59,59,.95)';g.shadowBlur=28;
    g.strokeText(turn.arrow,128,133);g.fillStyle='#fff';g.fillText(turn.arrow,128,133);
    mesh.userData.tex.needsUpdate=true;
  }
  function makeCheckpointGate(color,opacity) {
    const g=new THREE.Group(),mat=addMat(color,opacity),dimMat=addMat(color,opacity*.42);
    const postGeo=shared('raceGatePost',()=>new THREE.BoxGeometry(.085,1.22,.085));
    const beamGeo=shared('raceGateBeam',()=>new THREE.BoxGeometry(2.08,.085,.085));
    const footGeo=shared('raceGateFoot',()=>new THREE.BoxGeometry(.24,.05,.24));
    for(const side of [-1,1]){
      const p=new THREE.Mesh(postGeo,mat);p.position.set(side,.61,0);g.add(p);
      const f=new THREE.Mesh(footGeo,dimMat);f.position.set(side,.025,0);g.add(f);
    }
    const beam=new THREE.Mesh(beamGeo,mat);beam.position.set(0,1.22,0);g.add(beam);
    const halo=new THREE.Mesh(shared('raceGateHalo',()=>new THREE.TorusGeometry(1,.055,6,24)),dimMat);
    halo.position.y=.62;g.add(halo);
    g.userData.beam=beam;g.userData.halo=halo;g.frustumCulled=false;g.visible=false;
    return g;
  }
  function ensureRings() {
    if (races.rings) return races.rings;
    const cur=makeCheckpointGate(0xff3b3b,.96),nxt=makeCheckpointGate(0x20e3ff,.34);
    const arrow=makeCheckpointArrow();cur.add(arrow);
    raceGroup.add(cur,nxt);
    races.rings={cur,next:nxt,inactive:[],arrow};
    return races.rings;
  }
  function ensureInactiveGates(count){
    const rings=ensureRings();
    while(rings.inactive.length<count){const gate=makeCheckpointGate(0x20e3ff,.22);rings.inactive.push(gate);raceGroup.add(gate);}
    return rings.inactive;
  }
  function hideRaceVisuals(){
    if(races.rings){races.rings.cur.visible=false;races.rings.next.visible=false;for(const gate of races.rings.inactive)gate.visible=false;}
    if(races.ui&&races.ui.countdown)races.ui.countdown.style.display='none';
  }

  function placeRing(mesh, cp) {
    if (!cp) { mesh.visible = false; return; }
    mesh.visible = true;
    mesh.position.set(cp.x, cp.y + .15, cp.z);
    mesh.rotation.set(0, cp.heading, 0);
    mesh.scale.setScalar(cp.r);
    mesh.userData.baseR=cp.r;
  }

  function updateCheckpointVisuals(a) {
    if(!a||!races.rings)return;
    const cp=a.r.cps[a.cpIndex],rings=races.rings;
    if(cp&&rings.cur.visible){
      const pulse=1+Math.sin((a.t+a.countdown)*5.4)*.045;
      rings.cur.scale.setScalar(cp.r*pulse);
      if(rings.cur.userData.halo)rings.cur.userData.halo.rotation.z+=(1/60)*1.8;
    }
    const cyan=[rings.next,...rings.inactive];
    for(let i=0;i<cyan.length;i++)if(cyan[i].visible&&cyan[i].userData.halo)cyan[i].userData.halo.rotation.z+=(i&1?-1:1)*(1/60)*.55;
  }
  ...
  function buildRaces() {
    ...
    for (const def of data) {
      const bikes = api('bikes');
      const bikeOnly = bikes && Array.isArray(def.allowedVehicles) && def.allowedVehicles.length &&
        def.allowedVehicles.every(id => bikes.isBike(id));
      const poly = bikeOnly && def.anchors && def.anchors.length > 1
        ? def.anchors.map(a => ({ x: a.x, z: a.z, y: a.y || 0 }))
        : resolveAnchors('race:' + def.id, def.anchors);
  ...
  function buildStartLine(r) {
    ...
    for (let i = 0; i < ops.length; i++) {
      ...
      const mesh = takeCar(color, o.tuneKey);
  ...
  const POOL_MAX = 32;
  function takeCar(color, tuneKey) {
    const bikes=window.BikesModule;
    if(bikes&&bikes.isBike(tuneKey)){
      const mesh=bikes.createVehicleMesh(THREE,tuneKey,{color});mesh.userData.raceBike=true;mesh.visible=true;raceGroup.add(mesh);return mesh;
    }
  ...
  function giveCar(mesh) {
    if (!mesh) return;
    if (mesh.userData && mesh.userData.raceBike) { disposeTree(mesh); return; }
    mesh.visible = false;
  ...
  function ensureRaceUi() {
    ...
    const card = el('div', 'position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:min(92vw,470px);' +
    ...
    const raceSelect=el('select','width:100%;margin:11px 0 4px;padding:10px 11px;border:1px solid #59677d;border-radius:9px;background:#101725;color:#fff;font:800 12px/1 system-ui,sans-serif;letter-spacing:.04em',card);
    raceSelect.setAttribute('aria-label','Select race');
    const meta = el('div', 'font:600 12px/1.5 system-ui,sans-serif;color:#8fa4c8;margin:8px 0 12px', card);
    const field = el('div', 'font:600 12px/1.6 system-ui,sans-serif;color:#cfe0f7;margin-bottom:15px;max-height:190px;overflow:auto', card);
    ...
    const hud = el('div', 'position:absolute;right:14px;top:150px;min-width:220px;padding:10px 13px;display:none;' + PANEL);
    hud.id='raceHud';hud.style.borderColor = '#ffd23f';
    ...
    const hSplit=el('div','min-height:16px;font:800 11px/1.35 system-ui,sans-serif;color:#3bff8b;margin-top:4px',hud);hSplit.textContent='SPLIT —';
    const hBearing=el('div','display:flex;align-items:center;gap:7px;margin-top:6px;padding:5px 7px;border-radius:8px;background:rgba(32,227,255,.08);color:#bcefff;font:800 10px/1 system-ui,sans-serif;letter-spacing:.08em',hud);
    const hBearingArrow=el('span','display:inline-block;font:950 23px/1 system-ui,sans-serif;color:#20e3ff;transform-origin:50% 50%',hBearing);hBearingArrow.textContent='↑';
    const hBearingText=el('span','flex:1',hBearing);hBearingText.textContent='NEXT CHECKPOINT';
    const hWrong=el('div','display:none;margin-top:7px;padding:6px;border:1px solid #ff3b3b;border-radius:7px;background:rgba(255,59,59,.16);color:#ff7474;text-align:center;font:950 13px/1 system-ui,sans-serif;letter-spacing:.12em',hud);hWrong.textContent='WRONG WAY';
    const hTurn = el('div', 'width:104px;height:104px;margin:10px auto 0;border:4px solid #ff3b3b;border-radius:50%;background:radial-gradient(circle,rgba(255,59,59,.14),rgba(6,8,16,.88));display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;box-shadow:0 0 24px rgba(255,59,59,.35)', hud);
    hTurn.id='raceTurnCircle';hTurn.setAttribute('role','img');hTurn.setAttribute('aria-label','Next race direction: straight');
    const hTurnArrow=el('div','font:950 64px/.70 system-ui,sans-serif;color:#fff;text-shadow:0 0 14px #ff3b3b;transform-origin:50% 50%',hTurn);hTurnArrow.id='raceTurnArrow';hTurnArrow.textContent='↑';
    const hTurnText=el('div','font:950 9px/1 system-ui,sans-serif;color:#ff8d8d;letter-spacing:.13em;margin-top:8px',hTurn);hTurnText.id='raceTurnText';hTurnText.textContent='STRAIGHT';

    const countdown=el('div','position:absolute;inset:0;display:none;align-items:center;justify-content:center;pointer-events:none;z-index:68;background:radial-gradient(circle,rgba(4,8,16,.05),rgba(4,8,16,.34))',ctx.dom.ui);
    const countBox=el('div','min-width:260px;padding:24px 34px 22px;border:3px solid #ffd23f;border-radius:24px;background:rgba(5,8,16,.80);box-shadow:0 0 70px rgba(255,210,63,.36);text-align:center;transform:translateY(-8vh)',countdown);
    const countNumber=el('div','font:1000 clamp(92px,18vw,190px)/.78 system-ui,sans-serif;color:#ffd23f;text-shadow:0 0 28px #ff922b;letter-spacing:-.06em',countBox);countNumber.textContent='3';
    const countLabel=el('div','margin-top:18px;font:950 13px/1 system-ui,sans-serif;color:#eaf2ff;letter-spacing:.22em',countBox);countLabel.textContent='REV IN NEUTRAL · GRID LOCKED';

    const results = el('div', 'position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:min(92vw,440px);' +
      'padding:20px 22px;display:none;pointer-events:auto;' + PANEL);
    const rTitle = el('div', 'font:900 22px/1.15 system-ui,sans-serif;letter-spacing:.06em;color:#3bff8b', results);
    const rBody = el('div', 'font:600 12px/1.7 system-ui,sans-serif;color:#cfe0f7;margin:10px 0 14px;max-height:52vh;overflow:auto', results);
    const rRetry = el('button', 'width:100%;padding:11px;margin-bottom:8px;border-radius:9px;border:1px solid #ff3b3b;background:rgba(255,59,59,.14);color:#fff;font:800 13px/1 system-ui,sans-serif;letter-spacing:.08em;cursor:pointer', results);rRetry.textContent='RETRY SAME RACE';rRetry.style.display='none';
    ...
    races.ui = { card, title, raceSelect, meta, field, go, no, hud, hPos, hLap, hTime, hSplit, hBearing, hBearingArrow, hBearingText, hWrong, hTurn, hTurnArrow, hTurnText, countdown, countNumber, countLabel, results, rTitle, rBody, rRetry, rClose };
    raceSelect.addEventListener('change',()=>{const r=races.list.find(x=>x.def.id===raceSelect.value);if(r)renderRaceSummary(r,false);});
    go.addEventListener('click', () => startRace());
    no.addEventListener('click', () => closeSummary());
    rRetry.addEventListener('click', () => retryRace());
  ...
  const difficultyOf = d => clamp(d.difficulty == null ? Math.round(1 + Math.max.apply(null,(d.opponents||[{skill:.4}]).map(o=>o.skill||0)) * 4) : d.difficulty,1,5);
  const difficultyStars = d => '★'.repeat(difficultyOf(d)) + '☆'.repeat(5-difficultyOf(d));
  const raceType = d => d.raceType || ((d.laps||1)>1 ? 'CIRCUIT' : 'SPRINT');
  function fmtTime(t) {
  ...
  function firstBonusOf(d){return d.firstWinBonus==null?Math.round((d.reward||0)*.5):d.firstWinBonus;}

  function renderRaceSummary(r,refreshOptions) {
    const ui=ensureRaceUi(),d=r.def,res=saveGet('progression.raceResults.'+d.id,null);
    races.pending=r;
    if(refreshOptions!==false||!ui.raceSelect.options.length){
      ui.raceSelect.innerHTML=races.list.map(x=>'<option value="'+x.def.id+'">'+(x.def.name||x.def.id)+' · '+raceType(x.def)+' · '+difficultyStars(x.def)+'</option>').join('');
    }
    ui.raceSelect.value=d.id;
    ui.title.textContent=d.name||d.id;
    const firstOpen=!(res&&res.wins>0),firstBonus=firstBonusOf(d),winTotal=(d.reward||0)+(firstOpen?firstBonus:0);
    ui.meta.textContent=raceType(d)+' · '+(Math.round(r.length*(d.laps||1)/100)/10)+' km · '+(d.laps||1)+' lap'+((d.laps||1)>1?'s':'')+
      ' · '+(r.cps.length*(d.laps||1))+' checkpoints · '+difficultyStars(d)+
      ' · win $'+winTotal+(firstOpen&&firstBonus?' ($'+firstBonus+' first-win bonus)':'')+
      (d.entryFee?' · entry $'+d.entryFee:'')+(res&&res.best?' · PB '+fmtTime(res.best):'');
    ui.field.innerHTML=(d.opponents||[]).map(o=>
      ...
      (res?'<div style="margin-top:9px;color:#8fa4c8">records · '+(res.wins||0)+' wins / '+(res.runs||0)+' starts'+(res.bestPlace?' · best '+ordinal(res.bestPlace):'')+'</div>':'');
  }
  ...
  function openSummary(r) {
    ...
    if (zones.active) endZoneRun(false);
    setZoneMult(1);
    renderRaceSummary(r,true);
    ensureRaceUi().card.style.display='block';
  }
  ...
  function startRace(auto) {
    const r = races.pending || (auto ? races.retry : null);
    if (!r) return;
    const entryDamage=!auto&&ctx.engine.vehicleDamageSnapshot?ctx.engine.vehicleDamageSnapshot():null;
    if(auto&&ctx.engine.reviveForRace)ctx.engine.reviveForRace();
    const d=r.def;if(ctx.engine.ensurePlayerPossession)ctx.engine.ensurePlayerPossession('race-start',{x:r.start.x,z:r.start.z,y:r.start.y,heading:r.heading});
    ...
    const record=saveGet('progression.raceResults.'+d.id,null)||{};
    races.retry=r;races.active = {
      r: r, opponents: ops, laps: d.laps || 1, cpIndex: 0, lap: 0, t: 0, countdown: 3.999,
      playerS: 0, playerProgress: 0, playerLane: 0, missToast: 0,
      gridPose: { x: px, z: pz, h: h, y: ctx.world.groundHeightAt(px, pz, r.start.y) },
      finished: false, finishTime: 0, beeped: -1, autopilot: null,
      splits: [], bestSplits: Array.isArray(record.splits)?record.splits.slice():[], lastSplitT: 0,
      splitText:'SPLIT —',splitFlash:0,wrongWayT:0,wrongWay:false,wrongCpCd:0,goFlash:0,
      wantedAtStart:ctx.stats.wanted|0
    };
    const D = window.GAME_DEBUG_RACE;
    if (D && D.autopilot === true) {
    ...
    const playerMesh=ctx.player.carMesh;for(let i=ops.length-1;i>=0;i--)if(ops[i].mesh===playerMesh){ops.splice(i,1);}publishCollidables(ops);if(ctx.engine.ensurePlayerPossession)ctx.engine.ensurePlayerPossession('race-grid',{x:px,z:pz,y:ctx.world.groundHeightAt(px,pz,r.start.y),heading:h});if(entryDamage&&ctx.engine.restoreVehicleDamage)ctx.engine.restoreVehicleDamage(entryDamage,false);
    races.state='countdown';races.countdownClearClock=0;
    races.trafficExclusion={x:r.start.x,z:r.start.z,r:112,release:0};
    if(ctx.actors.clearTrafficZone)ctx.actors.clearTrafficZone(r.start.x,r.start.z,125);
    races.pending=null;ensureRings();updateRings();
    const ui=ensureRaceUi();ui.hud.style.display='block';ui.countdown.style.display='flex';ui.countNumber.textContent='3';ui.countNumber.style.color='#ffd23f';ui.countLabel.textContent='REV IN NEUTRAL · GRID LOCKED';
    evbus.emit('race:start',{raceId:d.id,worldId:worldId,laps:races.active.laps,opponents:ops.length});
  }

  function updateRings() {
    const a=races.active;if(!a)return;const rings=ensureRings(),cps=a.r.cps;
    placeRing(rings.cur,cps[a.cpIndex]);
    const inactive=ensureInactiveGates(Math.max(0,cps.length-2)),cyan=[rings.next,...inactive];let at=0;
    for(let i=0;i<cps.length;i++){if(i===a.cpIndex)continue;placeRing(cyan[at++],cps[i]);}
    while(at<cyan.length)cyan[at++].visible=false;
    paintCheckpointArrow(rings.arrow,raceTurnData(a));
  }

  function cleanupRaceEntities(a){
    if(!a)return;unpublishCollidables(a.opponents);for(const o of a.opponents)if(o.mesh&&o.mesh!==ctx.player.carMesh)giveCar(o.mesh);for(const mesh of a.r.parked)mesh.visible=true;hideRaceVisuals();races.trafficExclusion=null;races.countdownClearClock=0;
    const nav=api('nav');if(nav&&nav.clearCompassTarget)nav.clearCompassTarget();
  }
  function failRaceForRetry(reason){
    const a=races.active;if(!a)return;const retry=a.r;cleanupRaceEntities(a);races.retry=retry;races.active=null;races.state='results';
    const ui=ensureRaceUi();ui.hud.style.display='none';ui.rTitle.textContent='RACE FAILED';ui.rTitle.style.color='#ff3b3b';
    ui.rBody.innerHTML='<div>'+(reason||'Your vehicle was destroyed.')+'</div><div style="color:#8fa4c8;margin-top:8px">Retry restores the car, removes every old racer, rebuilds checkpoints, restarts the countdown and clears the starting grid.</div>';
    ui.rRetry.style.display='block';ui.results.style.display='block';evbus.emit('race:failed',{raceId:retry.def.id,worldId:worldId,reason:reason||'destroyed'});
  }
  function retryRace(){
    if(!races.retry)return;if(races.active){cleanupRaceEntities(races.active);races.active=null;}if(races.ui){races.ui.results.style.display='none';races.ui.rRetry.style.display='none';races.ui.card.style.display='none';}
    races.pending=races.retry;races.state='summary';startRace(true);
  }
  ...
  function driveAgent(o, a, dt, cheap) {
    ...
    const rubber=difficultyOf(a.r.def)>=5?Math.max(RUBBER_BAND,a.r.def.rubberBand||0):
      (a.r.def.rubberBand==null?RUBBER_BAND:a.r.def.rubberBand);
    target *= 1 + clamp((a.playerProgress - (o.lap * total + o.s)) / 900, -1, 1) * rubber;
    ...
    const driveVx=Math.sin(o.heading)*o.speed+o.shoveX,driveVz=Math.cos(o.heading)*o.speed+o.shoveZ;
    const mask=ctx.actors.DYNAMIC_MASK&&ctx.actors.DYNAMIC_MASK.EXTRA;
    if(ctx.actors.moveCircleWorld&&mask){
      const moved=ctx.actors.moveCircleWorld(o,driveVx,driveVz,dt,3.75,mask);
      const resolved=Math.hypot(moved.vx,moved.vz);
      if(moved.hit&&resolved>2){const travel=Math.atan2(moved.vx,moved.vz);o.heading+=clamp(angDiff(travel,o.heading),-.28,.28)*.35;}
      o.speed=Math.min(o.speed,resolved+2.5);
    }else{o.x+=driveVx*dt;o.z+=driveVz*dt;}
  ...
  function updateRace(dt) {
    ...
    if(races.state==='countdown'){
      if(ctx.engine.playerPossessionValid&&!ctx.engine.playerPossessionValid())ctx.engine.ensurePlayerPossession('race-countdown',{x:a.gridPose.x,z:a.gridPose.z,y:a.gridPose.y,heading:a.gridPose.h});
      a.countdown-=dt;placeCar(a.gridPose.x,a.gridPose.z,a.gridPose.h,a.gridPose.y);
      races.countdownClearClock-=dt;if(races.countdownClearClock<=0){races.countdownClearClock=.18;if(ctx.actors.clearTrafficZone)ctx.actors.clearTrafficZone(a.r.start.x,a.r.start.z,125);}
      const n=Math.max(1,Math.ceil(a.countdown));ui.countdown.style.display='flex';ui.countNumber.textContent=String(n);ui.countNumber.style.color='#ffd23f';ui.countLabel.textContent='REV IN NEUTRAL · GRID LOCKED';
      if(n!==a.beeped&&n>0&&n<=3){a.beeped=n;ctx.fx.banner(String(n),a.r.def.name||'','#ffd23f');ctx.audio.beep(520,.14,'square',.16);}
      if(a.countdown<=0){races.state='racing';a.t=0;a.goFlash=.72;if(races.trafficExclusion)races.trafficExclusion.release=1.25;ui.countNumber.textContent='GO';ui.countNumber.style.color='#3bff8b';ui.countLabel.textContent=a.r.def.name||'RACE';ctx.fx.banner('GO',a.r.def.name||'','#3bff8b');ctx.audio.beep(880,.28,'square',.18);const nav=api('nav');if(nav&&nav.setCompassTarget)nav.setCompassTarget(a.r.cps[0].x,a.r.cps[0].z,'#ff3b3b');}
      updateCheckpointVisuals(a);updateRaceHud(ui,dt);return;
    }
    if(races.state!=='racing')return;
    if(ctx.player.dead||ctx.player.dying){failRaceForRetry('Driver incapacitated.');return;}
    if(ctx.carState.hp<=0||ctx.carState.burning){failRaceForRetry('Race vehicle destroyed.');return;}
    if(a.goFlash>0){a.goFlash-=dt;if(a.goFlash<=0)ui.countdown.style.display='none';}
    if(races.trafficExclusion&&races.trafficExclusion.release>0){races.trafficExclusion.release-=dt;if(races.trafficExclusion.release<=0)races.trafficExclusion=null;}

    a.t+=dt;if(a.splitFlash>0)a.splitFlash-=dt;if(a.wrongCpCd>0)a.wrongCpCd-=dt;
    updateCheckpointVisuals(a);
    ...
    const motionHeading=ctx.player.speed<-1?ctx.player.heading+Math.PI:ctx.player.heading;
    const wrongNow=ctx.player.mph>10&&Math.cos(angDiff(motionHeading,ph))<WRONG_WAY_COS;
    a.wrongWayT=wrongNow?a.wrongWayT+dt:Math.max(0,a.wrongWayT-dt*2.2);
    a.wrongWay=a.wrongWayT>WRONG_WAY_GRACE;

    const cp = a.r.cps[a.cpIndex];
    const dx = ctx.player.x - cp.x, dz = ctx.player.z - cp.z;
    if(dx*dx+dz*dz<cp.r*cp.r&&Math.abs(ctx.player.y-cp.y)<14)takeCheckpoint(a);
    else{
      let wrong=-1;for(let i=0;i<a.r.cps.length;i++){if(i===a.cpIndex)continue;const q=a.r.cps[i],qx=ctx.player.x-q.x,qz=ctx.player.z-q.z;if(qx*qx+qz*qz<q.r*q.r&&Math.abs(ctx.player.y-q.y)<14){wrong=i;break;}}
      if(wrong>=0&&a.wrongCpCd<=0){a.wrongCpCd=1.6;ctx.fx.toast('✖ CHECKPOINT '+(wrong+1)+' INACTIVE · TARGET '+(a.cpIndex+1),'#20e3ff');}
      const ahead=dx*cp.fx+dz*cp.fz,lateral=Math.abs(dx*cp.fz-dz*cp.fx);if(ahead>CP_MISS_AHEAD&&lateral<cp.r*4&&a.missToast<=0){a.missToast=3.5;ctx.fx.toast('⚠ MISSED TARGET CHECKPOINT · TURN BACK','#ff6b6b');}
    }
  ...
  function takeCheckpoint(a) {
    const splitIndex=a.lap*a.r.cps.length+a.cpIndex;
    const cumulative=a.t,segment=cumulative-a.lastSplitT,pb=a.bestSplits[splitIndex];
    a.splits[splitIndex]=+cumulative.toFixed(3);a.lastSplitT=cumulative;
    const delta=Number.isFinite(pb)?cumulative-pb:null;
    a.splitText='CP '+(splitIndex+1)+' · '+fmtTime(segment)+(delta==null?'':' · '+fmtDelta(delta)+' PB');
    a.splitFlash=SPLIT_SHOW_TIME;
    ctx.fx.toast('🏁 '+a.splitText,delta!=null&&delta<0?'#3bff8b':'#20e3ff');
    a.cpIndex++;
    ...
    updateRings();lastTurn=raceTurnData(a);pendingTurnKey='';turnHold=0;turnStableCp=a.cpIndex;
    const nav=api('nav');if(nav&&nav.setCompassTarget){const c=a.r.cps[a.cpIndex];nav.setCompassTarget(c.x,c.z,'#ff3b3b');}
  }
  ...
  let hudClock=0,lastTurn={key:'straight',label:'STRAIGHT',arrow:'↑'},pendingTurnKey='',turnHold=0,turnStableCp=-1;
  function raceTurnData(a){
    const cps=a.r.cps,n=cps.length,idx=a.cpIndex,cur=cps[idx];
    if(idx===n-1&&a.lap+1>=a.laps)return{key:'straight',label:'FINISH',arrow:'↑'};
    const prev=idx>0?cps[idx-1]:(a.lap>0?cps[n-1]:a.r.start),next=cps[(idx+1)%n];
    const approach=Math.atan2(cur.x-prev.x,cur.z-prev.z),depart=Math.atan2(next.x-cur.x,next.z-cur.z);let d=depart-approach;while(d>Math.PI)d-=Math.PI*2;while(d<-Math.PI)d+=Math.PI*2;const ad=Math.abs(d);
    if(ad>2.38)return{key:d>0?'uturn-r':'uturn-l',label:'U-TURN',arrow:d>0?'↷':'↶'};
    if(ad<.22)return{key:'straight',label:'STRAIGHT',arrow:'↑'};
    if(ad<.58)return d>0?{key:'slight-r',label:'SLIGHT RIGHT',arrow:'↗'}:{key:'slight-l',label:'SLIGHT LEFT',arrow:'↖'};
    return d>0?{key:'right',label:'RIGHT',arrow:'→'}:{key:'left',label:'LEFT',arrow:'←'};
  }
  function updateRaceHud(ui, dt) {
    ...
    hudClock = 1 / 6;
    const rows = standings(a),place=rows.findIndex(r => r.you) + 1;
    ui.hPos.textContent = ordinal(place) + ' / ' + rows.length;
    ...
    ui.hSplit.textContent=a.splitFlash>0?a.splitText:(a.bestSplits.length?'PB '+fmtTime(a.bestSplits[a.lap*a.r.cps.length+a.cpIndex]||NaN):'SPLIT —');
    ui.hSplit.style.color=a.splitText.indexOf('−')>=0?'#3bff8b':'#20e3ff';
    ui.hWrong.style.display=a.wrongWay?'block':'none';
    if(a.wrongWay)ui.hWrong.style.filter='brightness('+(1.05+Math.abs(Math.sin(a.t*7))*.55)+')';
    const cp=a.r.cps[a.cpIndex],bearing=Math.atan2(cp.x-ctx.player.x,cp.z-ctx.player.z),rel=angDiff(bearing,ctx.player.heading),dist=Math.hypot(cp.x-ctx.player.x,cp.z-ctx.player.z);
    ui.hBearingArrow.style.transform='rotate('+rel.toFixed(3)+'rad)';
    ui.hBearingText.textContent=Math.round(dist)+'m TO CP '+(a.cpIndex+1);
    const nextTurn=raceTurnData(a);
    if(turnStableCp!==a.cpIndex){turnStableCp=a.cpIndex;lastTurn=nextTurn;pendingTurnKey='';turnHold=0;}
    else if(nextTurn.key===lastTurn.key){pendingTurnKey='';turnHold=0;}
    else{if(pendingTurnKey!==nextTurn.key){pendingTurnKey=nextTurn.key;turnHold=0;}if((turnHold+=1/6)>.55){lastTurn=nextTurn;pendingTurnKey='';turnHold=0;}}
    ui.hTurnArrow.textContent=lastTurn.arrow;ui.hTurnText.textContent=lastTurn.label;ui.hTurn.setAttribute('aria-label','Next race direction: '+lastTurn.label.toLowerCase());
    ui.hTurn.style.borderColor=lastTurn.key==='straight'?'#20e3ff':'#ff3b3b';
    ui.hTurn.style.boxShadow=lastTurn.key==='straight'?'0 0 22px rgba(32,227,255,.28)':'0 0 28px rgba(255,59,59,.48)';
    ui.hTurnArrow.style.textShadow=lastTurn.key==='straight'?'0 0 14px #20e3ff':'0 0 16px #ff3b3b';
  }
  ...
  function finishRace(a) {
    ...
    const first = !(prev.wins > 0),newBest=!(prev.best>0)||a.t<prev.best;
    const best = newBest ? a.t : prev.best;
    const bestPlace=prev.bestPlace?Math.min(prev.bestPlace,place):place;
    saveSet(key, { best: +best.toFixed(2), wins: (prev.wins || 0) + (won ? 1 : 0), runs: (prev.runs || 0) + 1,
      bestPlace, splits:newBest?a.splits.slice():(Array.isArray(prev.splits)?prev.splits:[]) });

    const firstBonus=firstBonusOf(d),base=d.reward||0;
    const reward=won?base+(first?firstBonus:Math.round(base*.30)):
      (place===2?Math.round(base*.12):place===3?Math.round(base*.06):0);

    races.state = 'results';
    const ui = ensureRaceUi();
    ui.hud.style.display = 'none';
    ui.rTitle.textContent = won ? 'RACE WON — 1ST' : 'FINISHED — ' + ordinal(place);
    ui.rTitle.style.color = won ? '#3bff8b' : '#ffd23f';
    const splitRows=a.splits.map((t,i)=>{const prevT=i?a.splits[i-1]:0,pb=a.bestSplits[i],delta=Number.isFinite(pb)?t-pb:null;return '<div style="display:flex;justify-content:space-between;gap:12px"><span>CP '+(i+1)+'</span><span>'+fmtTime(t-prevT)+(delta==null?'':' <b style="color:'+(delta<0?'#3bff8b':'#ff8d8d')+'">'+fmtDelta(delta)+'</b>')+'</span></div>';}).join('');
    ui.rBody.innerHTML = rows.map((r, i) =>
      ...
      '</div>').join('') +
      '<div style="margin-top:9px;color:#8fa4c8">time ' + fmtTime(a.t) + ' · PB ' + fmtTime(best)+(newBest?' · NEW RECORD':'')+'</div>' +
      '<div style="color:' + (reward ? '#3bff8b' : '#8fa4c8') + '">payout ' +
      (reward ? '$' + reward + (won&&first&&firstBonus?' · includes $'+firstBonus+' first-win bonus':won&&!first?' · repeat-win bonus':' · podium purse') : 'none') + '</div>'+
      '<div style="margin-top:10px;padding-top:8px;border-top:1px solid rgba(255,255,255,.12);color:#aebfd4"><b>CHECKPOINT SPLITS</b>'+splitRows+'</div>';
    ui.rRetry.style.display='block';ui.results.style.display = 'block';
    ctx.fx.banner(won ? 'RACE WON' : ordinal(place), fmtTime(a.t), won ? '#3bff8b' : '#ffd23f');
    ...
    payReward(reward, 'race:finish', {
      ...
      firstWinBonus:won&&first?firstBonus:0,difficulty:difficultyOf(d),wantedAtStart:a.wantedAtStart
    });
    if (races.rings) { races.rings.cur.visible = false; races.rings.next.visible = false; }
    races.trafficExclusion=null;
    const nav = api('nav');
    if (nav && nav.clearCompassTarget) nav.clearCompassTarget();
  }
  ...
  function paint(g, proj, full) {
    const a = races.active;
    if (a) {
      drawPoly(g,proj,a.r.poly,'rgba(255,210,63,.85)',full?3:2);g.save();
      for(let i=0;i<a.r.cps.length;i++){const cp=a.r.cps[i];g.fillStyle=i===a.cpIndex?'#ff3b3b':'rgba(32,227,255,.72)';g.beginPath();g.arc(proj.x2(cp.x),proj.z2(cp.z),i===a.cpIndex?(full?6:4):(full?3.2:2.2),0,6.283);g.fill();}
      g.restore();
    }
    if (!full) return;
  ...
  GameSystems.register({
    ...
    init(c) {
      ...
      c.scene.add(root);
      evbus.on('player:died',()=>{if(races.active)failRaceForRetry('Driver incapacitated.');if(zones.active)endZoneRun(false);setZoneMult(1);});
      evbus.on('vehicle:stage',d=>{if(races.active&&d&&d.target==='player'&&d.stage==='exploded')failRaceForRetry('Race vehicle destroyed.');});
    ...
    api: {
      ...
      report() {
        return {
          ...
          races: races.list.map(r => ({ id: r.def.id, type:raceType(r.def), difficulty:difficultyOf(r.def), cps: r.cps.length, len: Math.round(r.length), laps: r.def.laps })),
      ...
      raceState() {
        ...
        return {
          ...
          cp: a.cpIndex, cps: a.r.cps.length, wrongWay:a.wrongWay, split:a.splitText, wantedAtStart:a.wantedAtStart,
      ...
      trafficExcludedAt(x,z){const e=races.trafficExclusion;return!!e&&((x-e.x)*(x-e.x)+(z-e.z)*(z-e.z)<e.r*e.r);},
      trafficExclusion(){const e=races.trafficExclusion;return e?{x:e.x,z:e.z,r:e.r,release:e.release||0}:null;},
      retryRace(){retryRace();},
