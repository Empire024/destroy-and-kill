...
(function () {
  ...
  // Maximum geometric lift in VERT (.16 + .09 + .03). The larger amplitudes
  // in FRAG only shade the analytic normal; they never move the surface.
  const RIPPLE_PEAK = 0.28;
  // The shore texture's alpha channel is a low-road film mask. A full CELL of
  // margin guarantees LinearFilter has reached zero before a deck edge even
  // when a 40-unit texel grid lands in the least favourable phase.
  const FILM_ROAD_MARGIN = 40;
  const FILM_ROAD_CLEAR = 0.45;
  const FILM_ROAD_BASE_MAX = SEA_Y + RIPPLE_PEAK + FILM_ROAD_CLEAR;
  ...
  const BEACH_BAND = 3;         // 120-unit land strip: enough room to read as a beach
  const BEACH_LO = -0.8;        // ground must sit in this window around the
  const BEACH_HI = 3.0;         // waterline — a clifftop 40 up is not a beach
  const SAND_LIFT = 0.10;       // clear the terrain without z-fighting
  const SAND_DIP = 0.38;        // gentle first-water-cell slope, not a cliff        // how far the waterline edge of the sand goes under
  ...
  // Guaranteed POI-facing beach openings. These are not a second coastline:
  // land cells still come from the shared shore field, but this contract widens
  // the selected sand band, cuts every generated barrier out of the view
  // corridor, and adds one submerged sand apron for readable shallows.
  const AUTHORED_BEACH_FRONTS = Object.freeze([
    Object.freeze({
  ...
  const VERT = [
    ...
    '  float ripple = sin( world.x * 0.0104 + world.z * 0.0046 + uTime * 0.42 ) * 0.16',
    '               + sin( world.x * 0.0212 - world.z * 0.0305 + uTime * 0.63 ) * 0.09',
    '               + sin( world.x * 0.0850 + world.z * 0.0510 + uTime * 1.70 ) * 0.030;',
    '  world.y += ripple;',
    '  vXZ = world.xz;',
  ...
  const FRAG = [
    ...
    // Shore red: 0 hard against land, 1 in open water. Alpha is 0 under
    // low/coplanar road footprints (with a raster-cell margin), so only the
    // transparent ripple film yields there; the depthless body remains under
    // the deck and is painted over by the opaque road.
    '  float shore = 1.0;',
    '  float roadFilm = 1.0;',
    '  if ( uHasShore > 0.5 ) {',
    '    vec2 uv = ( vXZ - uShoreRect.xy ) * uShoreRect.zw;',
    '    if ( all( greaterThanEqual( uv, vec2( 0.0 ) ) ) && all( lessThanEqual( uv, vec2( 1.0 ) ) ) ) {',
    '      vec4 shoreSample = texture2D( uShore, uv );',
    '      shore = shoreSample.r;',
    '      roadFilm = shoreSample.a;',
    '    }',
    '  }',
    '  float h = A1 * a1 + A2 * a2 + A3 * a3;',
    '  float depth = smoothstep( 0.025, 0.72, shore );',
    ...
    '  float alpha = ( mix( 0.20, 0.60, fres ) + spec + foam * 0.5 ) * roadFilm;',
  ...
  const inlandSurfaces = [];
  let waterClockStart = 0;
  ...
  function buildShoreField(world) {
    ...
    // Alpha is a no-allocation low-road mask derived from the actual RoadNet:
    // every authored low crossing/approach is covered automatically, while
    // elevated RIM/deck water keeps its animated film.
    const units = new Uint16Array(nx * nz);
    const tex = new Uint8Array(nx * nz * 4);
    const roadSegs = world.roadsRef && world.roadsRef.segs;
    const filmRoads = roadSegs && roadSegs.length ? new RoadShield(roadSegs, 0, CELL) : null;
    let guardedRoadCells = 0;
    for (let i = 0; i < units.length; i++) {
      const u = Math.min(65535, (dt[i] / 3) * CELL);
      units[i] = u;
      const v = Math.min(255, Math.round(u / SHORE_RANGE * 255));
      const ix = i % nx, iz = (i / nx) | 0;
      const wx = minX + (ix + 0.5) * CELL, wz = minZ + (iz + 0.5) * CELL;
      const guarded = !!(filmRoads && filmRoads.lowBlocks(wx, wz, FILM_ROAD_MARGIN, FILM_ROAD_BASE_MAX));
      if (guarded) guardedRoadCells++;
      tex[i * 4] = v; tex[i * 4 + 1] = v; tex[i * 4 + 2] = v; tex[i * 4 + 3] = guarded ? 0 : 255;
    }
    ...
    const grid = {
      ...
      guardedRoadCells: guardedRoadCells
    };
  ...
  function RoadShield(segs, extend, cell) {
    ...
    for (let i = 0; i < segs.length; i++) {
      ...
      const e = {
        ...
        ay: s.ay === undefined ? 0 : s.ay,
        by: s.by === undefined ? (s.ay === undefined ? 0 : s.ay) : s.by
      };
  ...
  /** Is a corridor at or below maxY? Used once while baking the shore texture,
   *  so the animated film cannot rise onto a low road. The hash already stamps
   *  every segment into every overlapped cell; querying one cell is complete. */
  RoadShield.prototype.lowBlocks = function (x, z, pad, maxY) {
    const c = this.cell, cx = Math.floor(x / c), cz = Math.floor(z / c);
    const a = this.map.get(cx * 73856093 ^ cz * 19349663); if (!a) return false;
    for (let i = 0; i < a.length; i++) {
      const e = a[i];
      let t = ((x - e.ax) * e.ux + (z - e.az) * e.uz) / e.len;
      t = t < 0 ? 0 : t > 1 ? 1 : t;
      const dx = x - (e.ax + e.dx * t), dz = z - (e.az + e.dz * t);
      const r = e.width * 0.5 + pad;
      if (dx * dx + dz * dz <= r * r && e.ay + (e.by - e.ay) * t <= maxY) return true;
    }
    return false;
  };
  ...
  function beachFrontAt(worldId, x, z, pad) {
    pad = pad || 0;
    for (let i = 0; i < AUTHORED_BEACH_FRONTS.length; i++) {
      const f = AUTHORED_BEACH_FRONTS[i];
      if (f.worldId === worldId && x >= f.minX - pad && x <= f.maxX + pad &&
          z >= f.waterZ - pad && z <= f.inlandZ + pad) return f;
    }
    return null;
  }
  ...
  function buildCoast(world, g) {
    ...
    for (let iz = 0; iz < nz; iz++) {
      for (let ix = 0; ix < nx; ix++) {
        ...
        const front = beachFrontAt(id, cx, cz, 0);
        const regular = wd >= 1 && wd <= BEACH_BAND && gy >= BEACH_LO && gy <= BEACH_HI;
        const authored = !!(front && wd >= 1 && wd <= BEACH_BAND + 1 &&
                            gy >= BEACH_LO && gy <= BEACH_HI + 1.5);
        if (!regular && !authored) continue;
        let bad = false;
        ...
        beach[i] = 1;
        cand.push(i);
        if (authored) frontageCells++;
      }
    }
    // Shared shoreline corners already dip below the waterline; keep sand
    // on selected beach cells instead of adding a full above-water apron.
    const sandCells=cand;
    ...
    function corner(ix, iz) {
      ...
      if (!cornDone[ci]) {
        ...
        let wet = 0, landY = 0, landN = 0;
        for (let dz = -1; dz <= 0; dz++) for (let dx = -1; dx <= 0; dx++) {
          const a = ix + dx, c = iz + dz;
          if (a < 0 || c < 0 || a >= nx || c >= nz) { wet++; continue; }
          const cell=c*nx+a;if(!g.mask[cell])wet++;else{landY+=g.ground[cell];landN++;}
        }
        const wetFrac=wet/4, slope=wetFrac, ease=slope;
        const gy=landN?landY/landN:world.groundHeightAt(wx,wz,0);
        const y=gy+((SEA_Y-SAND_DIP)-gy)*ease;cornY[ci]=landN?Math.min(gy,y)+SAND_LIFT*(1-ease):(SEA_Y-SAND_DIP);
        cornT[ci]=wetFrac;
      }
      return ci;
    }
    ...
    for (let k = 0; k < sandCells.length; k++) {
      const i = sandCells[k], ix = i % nx, iz = (i / nx) | 0;
    ...
    // A single submerged strip makes the selected frontage visibly run into
    // the shallows. It sits below the sea at both ends, so the existing body +
    // film tint and foam still own the final water colour.
    let frontageApronTris = 0;
    for (let fi = 0; fi < AUTHORED_BEACH_FRONTS.length; fi++) {
      const f = AUTHORED_BEACH_FRONTS[fi]; if (f.worldId !== id) continue;
      const slices = 8, step = (f.maxX - f.minX) / slices;
      for (let s = 0; s < slices; s++) {
        const x0 = f.minX + step * s, x1 = x0 + step;
        const jit = 0.86 + 0.08 * rnd();
        const wet = sandColor(1, jit), swash = sandColor(0.86, jit);
        sand.quadC(
          [x0, SEA_Y - SAND_DIP - 0.18, f.waterZ], [x0, SEA_Y - 0.02, f.shoreZ],
          [x1, SEA_Y - 0.02, f.shoreZ], [x1, SEA_Y - SAND_DIP - 0.18, f.waterZ],
          wet, swash, swash, wet);
        frontageApronTris += 2;
      }
    }
    ...
    for (let iz = 1; iz < nz - 1; iz++) {
      for (let ix = 1; ix < nx - 1; ix++) {
        ...
        if (!touches) continue;
        const cx = minX + (ix + 0.5) * CELL, cz = minZ + (iz + 0.5) * CELL;
        if (beachFrontAt(id, cx, cz, 24)) continue;
    ...
    const colliderList = [];
    let modules = 0, gaps = 0, routeSkips = 0, frontageSkips = 0, coastSealed = 0;
    for (let c = 0; c < chains.length; c++) {
      ...
      for (let i = 0; i < pts.length - 1; i++) {
        ...
        for (let s = carry; s < seg; s += MODULE_STEP) {
          ...
          if (beachFrontAt(id, px, pz, MODULE_LEN)) {
            frontageSkips++; sinceGap++; runLeft--; continue;
          }
    ...
    const coast = {
      ...
      stats: {
        beachCells: cand.length, sandCells: sandCells.length, sandTris: sand.tris(), chains: chains.length,
        modules: modules, gaps: gaps, routeSkips: routeSkips, sealedRoutes: coastSealed,
        frontageCells: frontageCells, frontageApronTris: frontageApronTris,
        frontageGaps: AUTHORED_BEACH_FRONTS.filter(f => f.worldId === id).length,
        frontageSkips: frontageSkips,
  ...
  function makeSurfaceUniforms(surfaceY, shoreTexture, rect) {
    ...
    if (shoreTexture && rect) {
      u.uShore.value = shoreTexture;
      u.uShoreRect.value.set(rect[0], rect[1], rect[2], rect[3]);
      u.uHasShore.value = 1;
    }
    return u;
  }

  function radialShoreTexture() {
    const n = 96, a = new Uint8Array(n * n * 4);
    for (let z = 0; z < n; z++) for (let x = 0; x < n; x++) {
      const ux = ((x + 0.5) / n - 0.5) * 2, uz = ((z + 0.5) / n - 0.5) * 2;
      const q = Math.hypot(ux, uz), v = Math.max(0, Math.min(1, (1 - q) * 4));
      const c = Math.round(v * 255), i = (z * n + x) * 4;
      a[i] = c; a[i + 1] = c; a[i + 2] = c; a[i + 3] = 255;
    }
    const tex = new THREE.DataTexture(a, n, n, THREE.RGBAFormat);
  ...
  function updateSurfaceMeshes(meshes, cam) {
    const now = (performance.now() - waterClockStart) * 0.001;
    for (let i = 0; i < meshes.length; i++) {
      const m = meshes[i], u = m.material.uniforms;
  ...
  /** A bounded inland body using the ocean's exact ripple/film/foam shader. */
  function createInlandSurface(o) {
    if (!THREE || !o || !o.parent) return null;
    const x = +o.x || 0, z = +o.z || 0, y = +o.y || 0;
    const rx = Math.max(1, +o.radiusX || 1), rz = Math.max(1, +o.radiusZ || 1);
    const group = new THREE.Group(), texture = radialShoreTexture();
    group.name = o.name || 'inland-water';
    const geo = new THREE.CircleGeometry(1, 64); geo.rotateX(-Math.PI / 2);
    const rect = [x - rx, z - rz, 1 / (rx * 2), 1 / (rz * 2)];
    const base = new THREE.Mesh(geo, new THREE.ShaderMaterial({
      uniforms: makeSurfaceUniforms(y, texture, rect), vertexShader: VERT, fragmentShader: FRAG,
      fog: true, depthTest: false, depthWrite: false, side: THREE.FrontSide
    }));
    base.name = group.name + '-body'; base.renderOrder = -900;
    const top = new THREE.Mesh(geo, new THREE.ShaderMaterial({
      uniforms: makeSurfaceUniforms(y, texture, rect), vertexShader: VERT, fragmentShader: FRAG,
      defines: { FILM: '' }, fog: true, transparent: true, depthTest: true, depthWrite: false,
      polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 2,
      side: THREE.FrontSide
    }));
    top.name = group.name + '-film'; top.renderOrder = -1;
    const meshes = [base, top];
    for (let i = 0; i < meshes.length; i++) {
      const m = meshes[i];
      m.position.set(x, y, z); m.scale.set(rx, 1, rz);
      m.castShadow = false; m.receiveShadow = false;
      group.add(m);
    }
    base.onBeforeRender = function (renderer, sc, cam) { updateSurfaceMeshes(meshes, cam); };
    group.userData.waterSurface = { x: x, z: z, y: y, radiusX: rx, radiusZ: rz, shader: 'GameSea-ripple-film' };
    o.parent.add(group); inlandSurfaces.push(group);
    return group;
  }
  ...
  window.GameSea = {
    y: SEA_Y,
    geometrySpec: Object.freeze({
      ripplePeak: RIPPLE_PEAK,
      filmRoadMargin: FILM_ROAD_MARGIN,
      filmRoadClear: FILM_ROAD_CLEAR,
      filmRoadBaseMax: FILM_ROAD_BASE_MAX,
      beachFronts: AUTHORED_BEACH_FRONTS
    }),
    createInlandSurface: createInlandSurface,
    ...
    create: function (three, scene, camera) {
      ...
      THREE = three;
      sceneRef = scene;
      waterClockStart = performance.now();

      const uniforms = function () {
        return makeSurfaceUniforms(SEA_Y, null, null);
      };
    ...
    info: function () {
      ...
      return body ? {
        ...
        guardedRoadCells: g ? g.guardedRoadCells : 0,
        inlandSurfaces: inlandSurfaces.length,
