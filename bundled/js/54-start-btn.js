...
function seizeEngineFromHeat(){if(engineSeized)return;engineSeized=true;engineSeizeCooldown=0;turboSpool=0;turboPsi=0;breakDriftCombo();setBanner('ENGINE SEIZED','LET IT COOL — RESTARTING SOON','#ff3b3b');playHeatWhoosh(1);beep(54,.55,'sawtooth',.25);savePowertrainCondition(true);}
function updateEngineHeat(dt,throttle){
  if(onFoot||dead||engineBlown)return;
  // A heat seize is a breakdown, not a wreck: the engine cools and restarts in
  // limp condition. This also un-bricks vehicles whose saved condition hit 0.
  if(engineSeized){engineHeatSeconds=Math.max(0,engineHeatSeconds-dt*1.2);engineSeizeCooldown+=dt;
    if(engineSeizeCooldown>=7){engineSeizeCooldown=0;engineCondition=Math.max(engineCondition,25);engineDamage=100-engineCondition;engineSeized=false;misfireSeverity=Math.max(misfireSeverity,.5);setBanner('ENGINE COOLED','LIMP TO A GARAGE — CONDITION '+Math.round(engineCondition)+'%','#ff922b');savePowertrainCondition(true);}
    return;}
  const tune=currentPowertrainProfile(),safe=engineSafeRpm(),lim=engineLimiterRpm(),heatTol=Math.max(.25,tune.heatTolerance||.6),cool=Math.max(.3,tune.coolingStrength||.6),load=clamp(Math.abs(throttle),0,1),overSafe=Math.max(0,(fakeRpm-safe)/Math.max(450,lim-safe));
  const stressed=!engineSeized&&!pendingGear&&shiftKick<=0&&load>.6&&fakeRpm>=safe;if(stressed)engineHeatSeconds+=dt*(.7+load*.55+overSafe*.9)/heatTol;else engineHeatSeconds=Math.max(0,engineHeatSeconds-dt*(fakeRpm<safe*.72?1.28:.58)*cool);
  const heatLimit=24*heatTol,wasHot=engineOverheated;engineOverheated=engineHeatSeconds>=heatLimit;if(engineOverheated&&!wasHot){engineHeatWarned=true;setBanner('ENGINE OVERHEATING','LIFT OR SHIFT','#ff922b');playHeatWhoosh(.85);}
  if(stressed&&overSafe>.35){const limTol=Math.max(.1,tune.limiterTolerance||.5),vulnerability=clamp((1.05-limTol)/.83,0,1.35),wear=dt*(.18+(overSafe-.35)*.82)*vulnerability*(tune.extremeTune?2.6:1.4);if(wear>0)damagePowertrain(wear,wear*.07,'REV LIMITER ABUSE');}
  if(engineOverheated){const severity=clamp((engineHeatSeconds-heatLimit)/Math.max(6,16*heatTol),0,1);if(stressed)damagePowertrain(dt*(.21+severity*1.15)/heatTol,dt*severity*.09,'HEAT DAMAGE');if(car){engineSmokeClock-=dt;if(engineSmokeClock<=0){spawnEngineSmoke(.38+severity*.62);engineSmokeClock=lerp(.22,.06,severity);}}heatWhooshClock-=dt;if(heatWhooshClock<=0){playHeatWhoosh(.35+severity*.55);heatWhooshClock=lerp(1.8,.68,severity);}}else{engineSmokeClock=0;heatWhooshClock=Math.min(heatWhooshClock,.5);}
