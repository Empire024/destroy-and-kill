    ...
    init(ctx){
      ...
      function render(){
        ...
        if(open.kind==='garage'){
          ...
          selection=clamp(selection,0,open.stock.length-1);open.stock.forEach((s,i)=>{const e=prog.entry(s.id),lore=window.VEHICLE_LORE&&window.VEHICLE_LORE[s.id],need=prereq(s),owned=prog.isOwned(s.id),row=document.createElement('div');row.className='row'+(i===selection?' sel':'');row.innerHTML='<div><b>'+vehicleName(s.id)+'</b><br><small>'+(e?e.class+' · '+e.drivetrain:'')+(need?' · LOCKED: '+need:'')+(lore?'<br>'+lore.tagline:'')+'</small></div><div>'+(owned?'OWNED':money(s.price))+'</div>';row.addEventListener('click',()=>{selection=i;render();});panel.list.appendChild(row);});const st=open.stock[selection];panel.actions.appendChild(button('G · BUY '+money(st.price),()=>buy(open),prog.isOwned(st.id)||!!prereq(st)));if(!ctx.player.onFoot&&!ctx.player.inAircraft)panel.actions.appendChild(button('U · TUNE CURRENT',()=>tuneCurrent(open),false));
        }
