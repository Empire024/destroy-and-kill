...
(function () {
  ...
  const PRESET_CLAMP = { lo: 0.72, hi: 2.25 };
  const TUNED_FIELDS = ['power', 'grip', 'steer', 'drift'];
  const RESET_FIELDS = ['power','grip','steer','drift','topSpeed','turboPush','maxPsi','reverseAccel'];
  ...
  function loadCatalogue() {
    ...
    for (const e of raw) {
      ...
      const snap = {};
      for (const f of RESET_FIELDS) snap[f] = live[f];
      snap.gearAccel = Array.isArray(live.gearAccel) ? live.gearAccel.slice() : [];
  ...
  function loadState() {
    ...
    if (t && typeof t === 'object') for (const k of Object.keys(t)) if (t[k] && typeof t[k] === 'object') { const preset=t[k].preset||'stock',installed=Array.isArray(t[k].installed)?t[k].installed.filter(x=>presets.has(x)):['stock',preset]; tuneCfg[k]={preset,installed:[...new Set(['stock',...installed,preset])]} ; }
    ...
    currentId=(typeof cur==='string'&&owned.has(cur))?cur:(owned.has('streetDrift')?'streetDrift':([...owned][0]||null));if(currentId&&!cur)persist(SP.current,currentId);
  }
  ...
  function installedFor(id){const cfg=tuneCfg[id]||(tuneCfg[id]={preset:'stock',installed:['stock']});if(!Array.isArray(cfg.installed))cfg.installed=['stock',cfg.preset||'stock'];if(!cfg.installed.includes('stock'))cfg.installed.unshift('stock');return cfg.installed;}
  function presetInstalled(id,presetId){return presetId==='stock'||installedFor(id).includes(presetId);}
  function presetCostFor(id,presetId){if(presetInstalled(id,presetId))return 0;const def=presets.get(presetId),e=byId.get(id);if(!def||!e)return Infinity;return Math.max(0,Math.round((def.cost||0)*(.82+(e.powerTier||1)*.14)/25)*25);}
  ...
  function profileFor(id) {
    return (window.VEHICLE_UPGRADE_PROFILES && window.VEHICLE_UPGRADE_PROFILES[id]) || {
      maxStage:1,engineQuality:.6,safeRpm:7200,limiterRpm:7600,limiterTolerance:.5,
      overRevTolerance:.5,heatTolerance:.6,coolingStrength:.6,transmissionStrength:.6,
      forcedInduction:'na',turboCompatible:false,superchargerCompatible:false,
      nitrousCompatible:false,nitrousStage:99,nitrousCapacity:0,mass:1400,
      powerBandStart:1800,powerBandPeak:5200,powerBandEnd:6900,autoShiftRpm:6500,wheelspin:1
    };
  }

  function presetAllowed(id,presetId){const def=presets.get(presetId),profile=profileFor(id);return !!def&&(!!def.extreme||(def.stage||0)<=(profile.maxStage||0));}
  function inductionFor(profile,stage,extreme){let induction=profile.forcedInduction||'na';if(induction==='na'&&stage>=2&&(profile.turboCompatible||profile.superchargerCompatible))induction=profile.preferredForcedInduction||(profile.superchargerCompatible?'supercharger':'turbo');if(extreme&&induction==='na'&&(profile.turboCompatible||profile.superchargerCompatible))induction=profile.preferredForcedInduction||(profile.superchargerCompatible?'supercharger':'turbo');return induction;}
  function hardwareFor(id,presetId){
    const def=presets.get(presetId)||presets.get('stock'),profile=profileFor(id),stage=def.stage||0,extreme=!!def.extreme,induction=inductionFor(profile,stage,extreme),out=[];
    if(presetId==='grip')return(def.hardware||[]).slice();if(presetId==='drift')return(def.hardware||[]).slice();
    if(stage>=1)out.push('intake','exhaust','ECU tune');
    if(stage>=2){out.push('fuel system','cooling','clutch');if(induction!=='na')out.push(induction,induction==='turbo'?'intercooler':'charge cooling');}
    if(stage>=3)out.push('engine internals','transmission','differential');
    const nitrous=!!profile.factoryNitrous||!!profile.nitrousCompatible&&(stage>=(profile.nitrousStage||99)||extreme);if(nitrous)out.push('nitrous system');
    if(extreme)out.push('unsafe ECU flash','overspeed boost controller','fuel override');
    return[...new Set(out)];
  }

  /** Reset every catalogue tune to its frozen factory snapshot, then derive the
   * current car from one data profile plus one installed package. Hardware never
   * stacks accidentally and incompatible parts are never invented by the UI. */
  function applyTuneFields(idOverride){
    for(const[id,snap]of baseTunes){const live=ctx.vehicles.TUNES[id];if(!live)continue;for(const f of RESET_FIELDS)live[f]=snap[f];if(snap.gearAccel&&snap.gearAccel.length)live.gearAccel=snap.gearAccel.slice();}
    const id=idOverride||currentId;if(!id)return;const presetId=presetFor(id),def=presets.get(presetId),live=ctx.vehicles.TUNES[id],snap=baseTunes.get(id),profile=profileFor(id);if(!def||!live||!snap)return;
    const stage=def.stage||0,extreme=!!def.extreme;
    for(const f of TUNED_FIELDS){const m=Math.min(PRESET_CLAMP.hi,Math.max(PRESET_CLAMP.lo,num(def.mult&&def.mult[f],1)));live[f]=snap[f]*m;}
    live.topSpeed=snap.topSpeed*num(def.mult&&def.mult.topSpeed,1);const thrust=extreme?1.22:1+stage*.04;live.gearAccel=snap.gearAccel.map((v,i)=>i?v*thrust:v);
    const induction=inductionFor(profile,stage,extreme);live.forcedInduction=induction;
    if(induction==='na'){live.maxPsi=0;live.turboPush=0;}else{const requested=stage>=3||extreme?profile.stage3Psi:stage>=2?profile.stage2Psi:snap.maxPsi;live.maxPsi=Math.max(.18,num(requested,snap.maxPsi||.35))*(extreme?1.42:1);live.turboPush=Math.max(.18,snap.turboPush||.35)*(1+stage*.13+(extreme?.52:0));}
    const nitrous=!!profile.factoryNitrous||!!profile.nitrousCompatible&&(stage>=(profile.nitrousStage||99)||extreme);
    live.hardwareStage=extreme?'EXTREME':stage;live.installedHardware=hardwareFor(id,presetId);live.engineName=profile.engineName||id;live.engineClass=profile.engineClass||'road';live.engineQuality=profile.engineQuality;live.safeRpm=profile.safeRpm;live.limiterRpm=(profile.limiterRpm||profile.safeRpm+500)+(extreme?350:stage>=3?120:0);live.idleRpm=profile.idleRpm||900;live.powerBandStart=profile.powerBandStart||1800;live.powerBandPeak=profile.powerBandPeak||5200;live.powerBandEnd=profile.powerBandEnd||6900;live.autoShiftRpm=profile.autoShiftRpm||Math.min(live.limiterRpm-350,live.powerBandEnd);live.wheelspin=profile.wheelspin||1;
    live.limiterTolerance=profile.limiterTolerance*(extreme?.28:1+stage*.07);live.overRevTolerance=profile.overRevTolerance*(extreme?.34:1+stage*.09);live.heatTolerance=profile.heatTolerance*(extreme?.42:1+stage*.11);live.coolingStrength=profile.coolingStrength*(stage>=2?1.22:1)*(extreme?.72:1);live.transmissionStrength=profile.transmissionStrength*(stage>=3?1.24:1)*(extreme?.46:1);live.mass=profile.mass||1400;live.extremeTune=extreme;live.nitrousInstalled=nitrous;live.nitrousCapacity=nitrous?(profile.nitrousCapacity||100):0;live.naSupercar=!!profile.naSupercar;
  }

  function applyFactoryTuneFields(id){
    if(!id||!baseTunes.has(id)||!ctx.vehicles.TUNES[id])return false;
    const had=Object.prototype.hasOwnProperty.call(tuneCfg,id),previous=tuneCfg[id];
    tuneCfg[id]={preset:'stock',installed:['stock']};
    try{applyTuneFields(id);}finally{if(had)tuneCfg[id]=previous;else delete tuneCfg[id];}
    return true;
  }
  ...
  function selectVehicle(id, opts) {
    ...
    currentId = id;
    applyTuneFields(id);                 // engine profile must exist before the raw mesh/powertrain switch
    try { (ctx.vehicles.selectRaw || ctx.vehicles.select)(id); }
    catch (err) { console.error('[progression] vehicle switch "' + id + '" threw', err); return false; }
    applyLook(id);
...
`;
  ...
  const radial = {
    ...
    show() {
      ...
      // menu is up.
      radial.keyHandler = ev => { if(window.OV_TEXT_ENTRY&&window.OV_TEXT_ENTRY()&&(ev.key||'').toLowerCase()!=='escape')return; if(window.OV_TEXT_ENTRY&&window.OV_TEXT_ENTRY()&&(ev.key||'').toLowerCase()!=='escape')return;
  ...
  const api = {
    ...
    factoryTune(id) { const snap=baseTunes.get(id); return snap?Object.assign({},snap,{gearAccel:snap.gearAccel&&snap.gearAccel.slice?snap.gearAccel.slice():snap.gearAccel}):null; },
    hydrateVehicle(id, opts) { if (!byId.has(id) || !ctx.vehicles.TUNES[id]) return false; if (opts && opts.syncCurrent) currentId = id; applyTuneFields(id); return true; },
    hydrateStockVehicle(id, opts) { if (!byId.has(id) || !ctx.vehicles.TUNES[id]) return false; if (opts && opts.syncCurrent) currentId = id; return applyFactoryTuneFields(id); },
    ...
    /** District dealerships can sell any catalogue vehicle at authored prices
     *  while still using the same wallet, ownership set and save format. */
    dealerPurchase(id, authoredCost, opts) {
      const e = byId.get(id), options = opts || {};
      if (!e) return { ok: false, reason: 'unknown car' };
      if (owned.has(id)) return { ok: false, reason: 'already owned' };
      if (!options.ignoreUnlock && !api.isUnlocked(id)) return { ok: false, reason: progressFor(id).need };
      const cost = Math.max(0, Math.round(num(authoredCost, e.purchaseCost || 0)));
      if (cost <= 0) return { ok: false, reason: 'not for sale' };
      if (!spend(cost, 'dealer:' + (options.source || 'unknown') + ':' + id))
        return { ok: false, reason: 'need ' + nf(cost - wallet) + ' more' };
      unlocks[id] = true;
      owned.add(id);
      saveOwned(); saveUnlocks(); refreshWalletLine();
      if (cardsBuilt) renderCards();
      GameSystems.events.emit('vehicle:purchased', { id, cost, dealerId: options.source || null });
    ...
    presetAllowed(id, presetId) { return presetAllowed(id, presetId); },
    upgradeProfile(id){return Object.assign({},profileFor(id));},
    presetInstalled(id,presetId){return presetInstalled(id,presetId);},
    presetCost(id,presetId){return presetCostFor(id,presetId);},
    presetHardware(id,presetId){return hardwareFor(id,presetId);},
    installPreset(id,presetId,opts){const options=opts||{};if(!byId.has(id)||!presets.has(presetId)||!presetAllowed(id,presetId))return{ok:false,reason:'incompatible package'};const cost=presetCostFor(id,presetId);if(cost&&!options.paid&&!spend(cost,'tune:'+id+':'+presetId))return{ok:false,reason:'need '+nf(cost-wallet)+' more',cost};const list=installedFor(id);if(!list.includes(presetId))list.push(presetId);tuneCfg[id]={preset:presetId,installed:list.slice()};persist(SP.tunes,Object.assign({},tuneCfg));applyTuneFields();refreshWalletLine();return{ok:true,cost};},
    ...
    setPreset(id, presetId, opts) {
      if (!byId.has(id) || !presets.has(presetId) || !presetAllowed(id, presetId)) return false;
      const installed=installedFor(id).slice();tuneCfg[id]={preset:presetId,installed};
