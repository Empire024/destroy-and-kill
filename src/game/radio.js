...
(function () {
  ...
  function masterTarget() {
    // The engine's global mute (U) covers the radio too — a player who mutes the
    // game and still hears music has found a bug, not a feature.
    if (selfMuted || (ctx.audio && ctx.audio.muted)) return 0;
    return volume;
  }
  ...
  function persist() {
    var s = save();
    if (!s) return;
    s.set('prefs.radioStation', index < 0 ? null : stations[index].id);
  ...
  function paint() {
    ...
    if (index < 0) {
      ...
      nameEl.style.color = st.color || '#20e3ff';
      trackEl.textContent = !unlocked ? 'press a key to start audio'
        : (current && current.gen ? (current.gen.patternName || st.tagline) : st.tagline);
    }
    muteEl.textContent = (selfMuted || (ctx.audio && ctx.audio.muted)) ? '🔇' : '🔊';
    muteEl.classList.toggle('muted', selfMuted);
    powerEl.style.color = index < 0 ? '#7b8ea3' : '#3bff8b';
  }
  ...
  window.GAME_DEBUG_RADIO = {
    ...
    state: function () {
      return {
        ...
        ducking: duckTimer > 0, fadingOut: fading.length,
  ...
  window.GameSystems && window.GameSystems.register({
    ...
    init: function (context) {
      ...
      index = armedIndex;   // shown in the panel, but silent until a gesture
