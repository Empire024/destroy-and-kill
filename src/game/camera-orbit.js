...
(function () {
  ...
  let shake = 0, shakePeak = 0, canWriteShake = false;
  ...
  function frame(dt) {
    ...
    // Not ours: bonnet is welded to the car, on-foot has its own rig, and while
    // a selection screen is up the engine does not want a driving camera at all.
    if (mode === 1 || ctx.player.onFoot || ctx.engine.selectionOpen) return false;

    const cam = ctx.camera, keys = ci.keys || {};
    ...
    const targetFov = 62 + clamp((mph - 90) / 380, 0, 1) * 30 + (keys['shift'] ? 7 : 0);
    ...
    let ox, oy, oz;
    if (mode === 0) { ox = -chaseX * 24; oy = 13; oz = -chaseZ * 24; }
    else if (mode === 2) {
      const rx = Math.cos(cs.heading), rz = -Math.sin(cs.heading);
      ox = -chaseX * 20 + rx * 12; oy = 15; oz = -chaseZ * 20 + rz * 12;
    } else { ox = -chaseX * 42; oy = 27; oz = -chaseZ * 42; }
    ...
    const ax = cs.x, ay = cs.y + 2, az = cs.z;
    ...
    const look = 8 * Math.cos(orbitYaw);
    ci.camDesired.set(px, py, pz);
    ci.camTarget.set(cs.x + chaseX * look, cs.y + 4, cs.z + chaseZ * look);
