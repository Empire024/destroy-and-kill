...
(function () {
  ...
  function buildGraph(world) {
    ...
    for (let i = 0; i < segs.length; i++) {
      const s = segs[i];
      // Cache authored ambient weights once with the graph. Consumers read these hot-path-safe scalars.
      s._trafficWeight=Number.isFinite(s.trafficDensity)?Math.max(0,Math.min(1,s.trafficDensity)):1;
      s._policeWeight=Number.isFinite(s.policeWeight)?Math.max(0,Math.min(1,s.policeWeight)):1;
   ...
   */
  function randomPointOnRoads(nearX, nearZ, minDist, maxDist, weightKey) {
    const g = current(); if (!g) return null;
    minDist = minDist || 0;
    maxDist = maxDist || 600;
    ...
    for (let tries = 0; tries < 60; tries++) {
      ...
      const e = g.edges[ei], na = g.nodes[e.a], nb = g.nodes[e.b];
      const weight=weightKey&&e.seg&&Number.isFinite(e.seg[weightKey])?e.seg[weightKey]:1;
      if(weight<=0||(weight<1&&Math.random()>weight))continue;
      const t = Math.random();
