  ...
  GameSystems.register({
    ...
    update(dt, ctx) {
      ...
      if (best !== activePrompt) {
        activePrompt = best;
        if (best) {
          el.textContent = (document.body.classList.contains('mobile-ui') ? 'TAP · ' : '⏎  ') + best.label;
    ...
    api: {
      ...
      setLabel(id, text) {
        ...
        if (!p) return;
        p.label = text;
        if (activePrompt === p && el) el.textContent = (document.body.classList.contains('mobile-ui') ? 'TAP · ' : '⏎  ') + text;
      },
      active(){return activePrompt;},trigger(){if(!activePrompt)return false;trigger();return true;}
    }
  });
