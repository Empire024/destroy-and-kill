...
(function () {
  ...
  function killNpc(ctx,obj,opts) {
    ...
    ctx.events.emit('vehicle:stage',{target:obj,stage:'exploded',integrity:0,x,z,y,perpetrator:opts&&(opts.from==='player-weapon'||opts.from==='player')?'player':'environment',causeEventId:opts&&opts.causeEventId||null});
  }
  ...
  function damage(target, opts) {
    ...
    let amount = Math.max(0, +opts.amount || 0);const upgrades=GameSystems.api('vehicleUpgrades');if((target==='player'||target===ctx.player)&&upgrades&&upgrades.damageMultiplier)amount*=upgrades.damageMultiplier(null,opts.channel||'ballistic');
    if (!amount) return null;
    const channel = opts.channel || 'ballistic';

    if (target === 'player' || target === ctx.player) {
      const admin=GameSystems.api('admin');if(admin&&((admin.invincible&&admin.invincible())||(admin.invincibleCar&&admin.invincibleCar())))return{stage:player.stage,integrity:integrityOf(),applied:0};
      if (ctx.player.dead || ctx.player.dying) return null;
      if(channel==='collision'||channel==='explosion'){
        const before=clamp(ctx.carState.hp,0,100),applied=Math.min(before,amount);ctx.carState.hp=clamp(before-applied,0,100);player.collision=ctx.carState.hp;
        return{stage:player.stage,integrity:integrityOf(),applied};
      }
      const before=player.ballistic;player.ballistic=clamp(player.ballistic-amount,0,BALLISTIC_POOL);
      return{stage:player.stage,integrity:integrityOf(),applied:before-player.ballistic};
    }

    if (!target || target._bDead || target.dead) return null;
    const before=npcPool(ctx,target),applied=Math.min(before,amount),left=before-applied;
    target._bHp=left;
    if(opts.from==='player-weapon'||opts.source==='weapon')ctx.events.emit('damage:dealt',{amount:applied,x:opts.x===undefined?target.x:opts.x,y:opts.y===undefined?(target.y||0)+2:opts.y,z:opts.z===undefined?target.z:opts.z,kind:'vehicle',critical:!!opts.critical,target,source:'weapon'});
    if(left<=0){killNpc(ctx,target,opts);return{stage:'exploded',integrity:0,applied};}
    ...
    return { stage: stage, integrity: Math.round(frac * 100), applied: applied };
  }
  ...
  function snapshotPlayerDamage(){return{ballistic:player.ballistic,collision:player.collision,stage:player.stage};}
  function restorePlayerDamage(snap){
    const ctx=ctxRef,s=snap&&typeof snap==='object'?snap:null;player.ballistic=clamp(s&&s.ballistic!==undefined?s.ballistic:BALLISTIC_POOL,0,BALLISTIC_POOL);player.collision=clamp(ctx?ctx.carState.hp:100,0,100);player.stage=stageFor(integrityOf());player.wasBurning=false;player.fuseTaken=false;player.pendingBurn=false;player.klaxon=0;player.smokeClock=0;paintUI(player.stage,integrityOf());return snapshotPlayerDamage();
  }
  ...
  GameSystems.register({
    ...
    init(ctx) {
      ...
      ctx.events.on('player:died',d=>{clearPuffs();if(!d||!d.busted)resetPlayer(ctx,'died');});
    },
    ...
    update(dt, ctx) {
      ...
      if (cs.burning) {
        ...
        if(cs.fuse<=.9&&!ctx.player.onFoot&&ctx.player.carMesh&&ctx.engine.forceEjectBurningVehicle){ctx.fx.toast('AUTO-EJECT · VEHICLE ABOUT TO DETONATE','#ff922b');ctx.engine.forceEjectBurningVehicle();}
        if(cs.fuse<=.18){player.stage='exploded';if(ui){ui.classList.remove('show');uiShown=false;}ctx.events.emit('vehicle:stage',{target:'player',stage:'exploded',integrity:0,x:ctx.player.x,z:ctx.player.z});player.wasBurning=false;}
    ...
    api: {
      damage: damage,
      repair: repair,
      snapshot: snapshotPlayerDamage,
      restore: restorePlayerDamage,
      stage(target) {
        const ctx = ctxRef;
