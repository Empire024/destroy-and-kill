 ...
 * ----------------------------------------------------------------------------
 * Hand-authored, fixed-size arcade driving city (~11.5km x 11.3km of playable area).
 *
...
(function () {
  ...
  Builder.prototype.box = function (o) {
    ...
    if (!noCollide) {
      ...
      if (o.massClass) col.massClass = o.massClass;
      if (brk) {
  ...
  Builder.prototype.landmark = function (name, x, z, heading) { this._landmarks.push({ name, x, z, heading: heading || 0 }); return this; };
  ...
  function createNeonWorld(ctx) {
    ...
    if (county && !builder._saCounty) {
      try {
        county.build(builder);
        built.push(county.id);
      } catch (err) {
        console.error('[neon-city] San Andreas County failed to build:', err);
      }
    }
    builder.finish();
    ...
    const world = {
      ...
      groundHeightAt(x, z, curY, preferDeck) {
        ...
        if (s && (preferDeck || !s.outOfRange) && s.y >= base - 0.5) return s.y;
        return base;
      },
       ...
       * short-circuits the road-corridor test. Downtown, the docks, retail,
       * Northstar airport, Tidelight Island and Hills City are intentionally
       * poured edge to edge. Everything else answers null and the engine falls
       * back to distance-from-road: open hills, quarry dirt and the outer coast
       * remain genuinely loose surfaces.
       */
      surfaceTypeAt(x, z) {
        ...
        if (x > 650 && x < 5350 && z > -5350 && z < -2450) return 'paved';    // Northstar airport
        if (x > -1650 && x < 1550 && z > 4300 && z < 5780) return 'paved';    // Tidelight island
        if (x > -5820 && x < -4250 && z > -2520 && z < 720) return 'paved';   // Hills City
        return null;
      },
      ...
      updateAtmosphere(x, z) {
        // subtle district tint without any pop. Expansion zones each carry a
        // restrained local grade so their silhouettes read before props do.
        const scene = ctx.scene;
        if (!world._fogTarget) world._fogTarget = new THREE.Color(0x120a20);
        let fogHex = 0x120a20;
        if (x > 650 && z < -2450) fogHex = 0x0a1730;                 // airport blue
        else if (z > 4250 && x > -1800 && x < 1750) fogHex = 0x071d2b; // marina cyan
        else if (x < -4200) fogHex = 0x1c1028;                     // villa dusk
  ...
  window.GameWorlds.register({
    id: 'neon',
    name: 'NEON STATE',
    tagline: 'Welcome to the state of neon — city core, airport, marina, hills and quarry.',
    accent: '#ff2d9b',
    fog: 0x120a20,
